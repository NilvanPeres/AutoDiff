...
adouble objectiveFunction(adouble x, adouble y)
{
  adouble t1 = -13.0 + x + (((5.0 - y) * y - 2.0) * y);
  adouble t2 = -29.0 + x + (((y + 1.0) * y - 14.0) * y);
...
//   // Configuração para rastrear as derivadas
//   trace_on(1);
//   for (int i = 0; i < n; i++)
//   {
//     xad_ad[i] <<= xad[i];
...
void sevalg(int n, double *x, double *g, int *inform)
{
  ...
  adouble af = objectiveFunction(xad_ad[0], xad_ad[1]);
  ...
  gradient(1, n, xad_ad, grad); // Calcular derivadas
